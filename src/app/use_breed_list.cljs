...
(defn use-breed-list [animal]
  (let [[breed-list set-breed-list!]   (uix/use-state [])
    ...
    (uix/use-effect
     (fn []
       (println "I'm in use effect. Animal is: " animal)
       (cond
         (empty? animal) (do (println "Animal is empty.")
                             (set-breed-list! []))
         (contains? local-cache animal) (do (println "Animal exists in cache: " animal)
                                            (set-breed-list! (get local-cache animal)))
         :else (do (println "I'm requesting breeds")
                   (request-breed-list
                    animal set-breed-list! set-status! local-cache set-local-cache!))))
     [animal])
    [breed-list status]))
